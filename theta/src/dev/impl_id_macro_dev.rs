...
impl Actor for Manager {
    ...
    //     match msg {
    //         CreateWorker { name } => {
    //             println!("Creating worker with name: {}", name);
    //             let worker = ctx.spawn(Worker {});
    //             self.workers.insert(name.clone(), worker.clone());
    ...
    //                 worker.raw_send(Nil, k).unwrap();
    //             } else {
    //                 k.send(Err("Worker not found".into())).unwrap();
    //             }
    //         }
    //     }
    //     .into()
    ...
    // async fn _(mut self, ctx: Context<Self>, msg: CreateWorker) -> () {
    ...
    // async fn _(mut self, ctx: Context<Self>, msg: GetWorker) -> Option<ActorRef<Worker>> {
    //     println!("Getting worker with name: {}", msg.name);
    //     if let Some(worker) = self.workers.get(&msg.name).cloned() {
