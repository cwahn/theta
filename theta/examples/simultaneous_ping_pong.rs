...
async fn main() -> anyhow::Result<()> {
    ...
    let (our_secret_key, other_public_key, child_secret_for_spawn) = if args.len() == 3 {
        ...
        info!("Received args[2] (our secret key bytes): {} chars", &args[2].len());
        ...
        info!("Parsed our_secret_key, public version: {}", our_secret_key.public());
        ...
        info!("Parent will connect to child. Parent will pass child's public key: {}", child_public);
        info!("Child will connect to parent. Child will receive parent's public key: {}", parent_public);
    ...
    if let Some(child_secret) = child_secret_for_spawn {
        ...
        info!("  - arg[1] (parent's public key for child to connect to): {}", our_secret_key.public());
        info!("  - arg[2] (child's secret key): {} bytes", child_secret_bytes.len());
        
        let _child = Command::new("./target/debug/examples/simultaneous_ping_pong")
            .args(&[
                &our_secret_key.public().to_string(),
                &child_secret_str
            ])
            .spawn()?;
    ...
    info!("Key comparison: our_key={}, other_key={}, our_key > other_key = {}", 
          public_key, other_public_key, public_key > other_public_key);
    ...
    let other_ping_pong = loop {
        info!("Attempting lookup {} for {}", retry_count + 1, ping_pong_url);
        match ActorRef::<PingPong>::lookup(&ping_pong_url).await {
            Ok(actor) => {
                info!("Successfully connected to peer after {} attempts", retry_count + 1);
                break actor;
            }
            Err(e) => {
                retry_count += 1;
                error!("Lookup attempt {} failed for URL: {ping_pong_url}. Error: {e}", retry_count);
        ...
        match current_actor_ref.ask(ping).await {
            Err(e) => {
                ...
                loop {
                    match ActorRef::<PingPong>::lookup(&ping_pong_url).await {
                        Ok(actor) => {
                            info!("Successfully reconnected after {} attempts", reconnect_attempts + 1);
            ...
            Ok(_pong) => {
                ...
                info!("received pong from {} in {elapsed:?}", current_actor_ref.id());
            }
        }
    }
