...
async fn main() -> anyhow::Result<()> {
    ...
    if let Err(e) = ActorRef::<PingPong>::lookup_local(b"iroh://ping_pong@{public_key}") {
        error!(
            "failed to lookup PingPong actor via URL: iroh://ping_pong@{public_key}. Error: {e}"
        );
        return Ok(());
    }
    info!("PingPong actor is bound and can be looked up via URL: iroh://ping_pong@{public_key}");
