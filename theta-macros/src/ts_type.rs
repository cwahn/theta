            ...
            match &v.fields {
                ...
                Fields::Unnamed(unnamed) => {
                    if unnamed.unnamed.len() == 1 {
                        // Single-field tuple variant → { VariantName: InnerType }
                        let ts_ty = rust_type_to_ts(&unnamed.unnamed.first().unwrap().ty);
                        format!("{{ {}: {} }}", variant_name, ts_ty)
                    } else {
                        // Multi-field tuple variant → { VariantName: [A, B, ...] }
...
fn generate_custom_section(ts_section: &str) -> TokenStream2 {
    quote! {
        #[cfg(all(feature = "ts", target_arch = "wasm32"))]
        #[allow(non_upper_case_globals)]
        const _: () = {
            #[::theta_ts::prelude::wasm_bindgen::prelude::wasm_bindgen(typescript_custom_section)]
            const TS_TYPE: &'static str = #ts_section;
        };
    }
}

// --- Public helpers for ts.rs ---
